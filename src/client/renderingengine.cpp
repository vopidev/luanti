		...
		int percent, float *indef_pos)
{
	...
	const float LOADING_SCREEN_FORMSPEC_WIDTH = 10.24f;   // Matches main menu formspec
	const float LOADING_SCREEN_FORMSPEC_HEIGHT = 5.12f;   // Matches main menu formspec
	const float LOADING_TEXT_PADDING_LEFT_PCT = 14.0f;    // Left padding as % of progress bar
	const float LOADING_TEXT_PADDING_BOTTOM_PCT = 11.0f;  // Bottom padding as % of progress bar
	const float LOADING_SCREEN_PADDING_FACTOR = 0.05f;    // Screen edge padding
	const video::SColor LOADING_TEXT_COLOR(255, 255, 139, 178);  // Kawaii pink color
	...
	if (bg) {
		...
		// Use max scale to ensure full coverage (background will be cropped if aspect differs)
		...
		v2u32 bg_size(
			(u32)(scale_factor * sourcesize.X),
			(u32)(scale_factor * sourcesize.Y)
		);

		// Center the background
		v2s32 offset(
			((s32)screensize.X - (s32)bg_size.X) / 2,
			((s32)screensize.Y - (s32)bg_size.Y) / 2
		);
	...
	// Calculate progress range (for indefinite progress animation)
	int percent_min = 0;
	...
	// Draw progress bar (custom VOPI Engine style)
	if ((percent_min >= 0) && (percent_max <= 100)) {
		...
		if (progress_img && progress_img_bg) {
			...
			// Calculate padded screen size (with screen edge padding)
			...
			// Draw simple progress rectangle
		...
		// No progress bar, just end scene
		guienv->drawAll();
		driver->endScene();
	}
#else
