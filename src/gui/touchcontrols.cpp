...
#if IS_VOPI_ENGINE
	// VOPI UI does not use the overflow menu button.
	if (id == overflow_id)
		return false;
#endif
...
void TouchControls::pushTouchableHudRects(std::vector<std::pair<u32, recti>> rects)
{
	...
	// Keep a held button's hit-rect in sync with the freshly-pushed list: an
	// anchor_above_hotbar button moves when the hotbar shows/hides, so the
	// frozen press-time rect would otherwise mis-track the tap. If the held id
	// is no longer present — the element was removed or is no longer a touchable
	// image (the list only contains those) — cancel the press without firing,
	// so a reused HUD slot can't deliver the click to a different element.
	// Re-evaluate "inside" against the finger's last known position so a button
	// moving out from under a stationary finger doesn't fire on release.
	if (m_has_hud_btn_id) {
		bool still_present = false;
		for (const auto &[id, rect] : m_touchable_hud_rects) {
			if (id == m_hud_btn_hud_id) {
				m_hud_btn_rect = rect;
				auto it = m_pointer_pos.find(m_hud_btn_id);
				if (it != m_pointer_pos.end())
					m_hud_btn_inside = rect.isPointInside(it->second);
				still_present = true;
				break;
			}
		}
...
void TouchControls::resetTapState()
{
	...
	// Mark the current touch as "moved" so a long tap is not re-detected —
	// prevents dig from starting after eating food while the finger is held.
	m_move_has_really_moved = true;

	// Release any pressed keys so actions don't continue after the item change.
	if (m_dig_pressed) {
		emitGameKeyEvent(id_to_action(dig_id), false);
		m_dig_pressed = false;
	}
